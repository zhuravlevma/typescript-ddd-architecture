...
import { UpdateOfferCommand, UpdateOfferInPort } from '../ports/in/update-offer.in-port';
...
export class UpdateOfferInteractor implements UpdateOfferInPort {
  ...
  async execute(updateOfferCommand: UpdateOfferCommand): Promise<OfferEntity> {
    try {
      const offer = await this.findOfferByIdPort.findOfferById(updateOfferCommand.offerId);
